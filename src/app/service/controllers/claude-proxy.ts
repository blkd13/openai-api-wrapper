 ...
 * Vertex AI の URL を生成
 */
function buildVertexUrl(baseApiPath: string, project: string, location: string, model: string, method: 'predict' | 'streamRawPredict'): string {
    const basePath = location === 'global' ? baseApiPath : `${location}-${baseApiPath}`;
    return `https://${basePath}/projects/${project}/locations/${location}/publishers/anthropic/models/${model}:${method}`;
}
...
async function handleError(
    ...
    if (predictLogger && historyContext) {
        ...
        try {
            ...
            console.error('Failed to persist predict history', JSON.stringify(logError, Utils.genJsonSafer()));
        }
    }
...
async function commonPreProcess(req: UserRequest, suffix: string) {
    ...
    const vertexUrl = buildVertexUrl((aiProvider.config as AnthropicVertexAIConfig).baseURL || `${GCP_API_BASE_PATH}/v1`, targetProject, targetLocation, modelName, suffix as any);
    ...
    async (_req: Request, res: Response) => {
        ...
        try {
            ...
            if (vertexResponse.data?.usage) {
                let usageCost = calcCost(vertexResponse.data.usage, aiPrice);
    ...
    async (_req: Request, res: Response) => {
        ...
        try {
            ...
            vertexResponse.data.on('end', async () => {
                ...
                if (usage) {
                    let usageCost = calcCost(usage, aiPrice);
                    tokenCount.prompt_tokens = usageCost.prompt_tokens;
                    tokenCount.completion_tokens = usageCost.completion_tokens;
                    tokenCount.cost = usageCost?.cost || 0;
                }
