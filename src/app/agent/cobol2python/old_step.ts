...
import { BaseStepCobol2Python, MultiStepCobol2Python, Step0020_ConvertToDoc } from "./runner.js";
...
export class Step0030_ClassifyDoc extends MultiStepCobol2Python {
    ...
    constructor() {
        ...
        this.childStepList = getStepInstance(Step0020_ConvertToDoc).childStepList.map(step0 => {
            ...
            return new Step0030_ClassifyDocChil(step.targetFilePath, step.innerIndex, step.sectionName, step.result);
        });
    }

    postProcess(resultList: string[]): string[] {
        const summary: Record<ArchitectureType, string[]> = { 'entity': [], 'use_case': [], 'application': [], 'interface_adapter': [], 'framework': [], 'driver': [], 'other': [] };
        resultList.forEach((result, index) => {
            ...
            Object.entries(flags).forEach(([key, value]) => {
                if (value === true) summary[key as ArchitectureType].push(this.childStepList[index].label);
            });
        });
        // console.log(summary);
        return resultList;
...
type ArchitectureType = 'entity' | 'use_case' | 'application' | 'interface_adapter' | 'framework' | 'driver' | 'other';
const architectureTypeList: ArchitectureType[] = ['use_case', 'interface_adapter', 'driver'];
...
class Step0040_RebuildDocument extends MultiStepCobol2Python {
    ...
    constructor() {
        ...
        class Step0040_RebuildDocumentChil extends BaseStepCobol2Python {
            ...
            systemMessageEn = 'An experienced and excellent software engineer. Specializes in domain-driven design and clean architecture.';
            // 共通定義を上書きする場合はここで定義する。
            constructor(public targetFilePath: string, public innerIndex: number, public sectionName: string, public document: string, public classifies: string[]) {
                super();
                const baseName = path.basename(targetFilePath);
                // 複数並列処理するので、被らないようにラベルを設定する。（これがログファイル名になる）
                this.label = Utils.safeFileName(`${this.constructor.name}_${baseName}-${innerIndex}-${sectionName.replaceAll('-', '_')}`); // Utils.safeFileNameはファイル名として使える文字だけにするメソッド。
        ...
        this.childStepList = docList.childStepList.map((step0, index) => {
            const step = step0 as any as { targetFilePath: string, innerIndex: number, sectionName: string, sectionCode: string, result: string, label: string };
            const classify = Utils.jsonParse(classifyList.childStepList[index].result) as Record<ArchitectureType, boolean>;
            const classifies = Object.entries(classify).filter(([key, value]) => architectureTypeList.includes(key as any) && value).map(([key, value]) => key);
            return new Step0040_RebuildDocumentChil(step.targetFilePath, step.innerIndex, step.sectionName, step.result, classifies);
    ...
    postProcess(resultList: string[]): string[] {
        ...
        getStepInstance(Step0030_ClassifyDoc).childStepList.forEach((step, index) => {
            const res = Utils.jsonParse(step.result) as Record<ArchitectureType, boolean>;
...
class Step0050_ extends MultiStepCobol2Python {
    ...
    constructor() {
        ...
        class Step0050_RebuildDocumentChil extends BaseStepCobol2Python {
            ...
            systemMessageEn = 'An experienced and excellent software engineer. Specializes in domain-driven design and clean architecture.';
            // 共通定義を上書きする場合はここで定義する。
            constructor(public targetFilePath: string, public innerIndex: number, public sectionName: string, public document: string, public classify: Record<string, boolean>) {
                super();
                const baseName = path.basename(targetFilePath);
                // 複数並列処理するので、被らないようにラベルを設定する。（これがログファイル名になる）
                this.label = Utils.safeFileName(`${this.constructor.name}_${baseName}-${innerIndex}-${sectionName.replaceAll('-', '_')}`); // Utils.safeFileNameはファイル名として使える文字だけにするメソッド。
                const classifyString = Object.entries(classify).filter(([key, value]) => !['other', 'descriptions'].includes(key) && value).map(([key, value]) => key).join(', ');
                // 個別の指示を作成。
                this.chapters = [
                    {
                        title: `Instructions`,
        ...
        this.childStepList = docList.childStepList.map((step0, index) => {
            const step = step0 as any as { targetFilePath: string, innerIndex: number, sectionName: string, sectionCode: string, result: string };
            const classify = Utils.jsonParse(classifyList.childStepList[index].result) as Record<string, boolean>;
            return new Step0050_RebuildDocumentChil(step.targetFilePath, step.innerIndex, step.sectionName, step.result, classify);
        }).filter(step => Object.entries(step.classify).filter(([key, value]) => value).length > 1);
    }
