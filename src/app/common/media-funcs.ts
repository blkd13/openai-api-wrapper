...
export function getMetaDataFromFile(filePath: string): Observable<FfprobeData> {
    // console.log(`getMetaDataFromFile=${filePath}`);
...
export function getMetaDataFromDataURL(dataURL: string, saveAs: string = ''): Observable<FfprobeData> {
    ...
    // console.log(`getMetaDataFromDataURL=${filePath}`);
...
export async function detectMimeType(filePath: string, fileName: string): Promise<string> {
    // console.log(`detectMimeType=${filePath}`);
...
export async function minimizeAudioForMinutes(inputFile: string, outputFile: string): Promise<string> {
    try {
        // console.log(`minimizeAudioForMinutes=${inputFile}`);
        // FFmpegコマンドを構築
...
export async function normalizeAndMinimizeAudio(inputFile: string, outputFile: string, outputFormat: 'opus' = 'opus'): Promise<string> {
    try {
        // console.log(`normalizeAndMinimizeAudio=${inputFile}`);
        // FFmpegコマンドを構築
        const command = `ffmpeg -i "${inputFile}" \
...
export async function minimizeVideoForMinutes(inputFile: string, outputFile: string, outputFormat: 'mp4' | 'webm' | '3gpp' = 'mp4'): Promise<string> {
    try {
        // console.log(`minimizeVideoForMinutes=${inputFile}`);
...
export async function convertAndOptimizeImage(inputFile: string, outputFile: string): Promise<string> {
    // console.log(`convertAndOptimizeImage=${inputFile},outputFile=${outputFile}`);
...
// unoconv は複数プロセス並列で動かすことができないので直列打ちする仕掛けを入れた。
let conversionQueue = Promise.resolve();
export async function convertPptxToPdf(inputPath: string, outputPath: string): Promise<void> {
    // console.log(`convertPptxToPdf=${inputPath}, outputFile=${outputPath}`);

    // conversionQueueに処理をつないでいくことで、逐次的な処理を保証する
    conversionQueue = conversionQueue.then(() => {
        return new Promise<void>((resolve, reject) => {
            const unoconv = (process.platform || '').startsWith('win')
                ? spawn('wsl', ['unoconv', '-f', 'pdf', '-o', outputPath.replaceAll(/\\/g, '/'), inputPath.replaceAll(/\\/g, '/')])
                : spawn('unoconv', ['-f', 'pdf', '-o', outputPath, inputPath]);
            ...
            unoconv.on('close', (code) => {
                if (code === 0) {
                    console.log(`PowerPoint file converted to PDF successfully! ${inputPath} => ${outputPath}`);
                    resolve();
                } else {
                    reject(new Error(`unoconv process exited with code ${code}`));
                }
            });
        });
    });

    // キューに蓄積された処理が完了するまで待つ
