...
export class MyAnthropicVertex extends MyAnthropicBase<AnthropicVertex> {
    constructor(public params: AnthropicVertexAIConfig[]) {
        ...
        this.clients = params.flatMap(param => param.regionList.map(region => {
            ...
            const baseURL = region === 'global' ? `https://${baseDomain}` : `https://${region}-${baseDomain}`;
            const httpAgent = getUndiciHttpProxy(baseURL);
            const option = {
                baseURL,
...
export function calcCost(usage: Anthropic.Usage, aiPrice: AIModelPricingEntity): { prompt_tokens: number; completion_tokens: number; cost: number } {
    ...
    // console.log(`Input tokens: ${usage.input_tokens}, Cost so far: $${usageCost.toFixed(6)}`);
    // 出力トークン
    usageCost += (usage.output_tokens || 0) * aiPrice.outputPricePerUnit / unit;
    // console.log(`Output tokens: ${usage.output_tokens}, Cost so far: $${usageCost.toFixed(6)}`);
...
// {"ephemeral_5m_input_tokens":  6.25,"ephemeral_1h_input_tokens":  10, "cache_read_input_tokens": 0.50}
// {"ephemeral_5m_input_tokens": 18.75,"ephemeral_1h_input_tokens":  30, "cache_read_input_tokens": 1.50}
// {"ephemeral_5m_input_tokens":  3.75,"ephemeral_1h_input_tokens":   6, "cache_read_input_tokens": 0.30}
// {"ephemeral_5m_input_tokens":  1.25,"ephemeral_1h_input_tokens":   2, "cache_read_input_tokens": 0.10}
// {"ephemeral_5m_input_tokens":  1.00,"ephemeral_1h_input_tokens": 1.6, "cache_read_input_tokens": 0.08}
// {"ephemeral_5m_input_tokens": 18.75,"ephemeral_1h_input_tokens":  30, "cache_read_input_tokens": 1.50}
// {"ephemeral_5m_input_tokens":  0.30,"ephemeral_1h_input_tokens": 0.5, "cache_read_input_tokens": 0.03}