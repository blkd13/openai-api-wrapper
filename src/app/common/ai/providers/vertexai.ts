...
function calcCost(tokenCount: TokenCount, aiPrice: AIModelPricingEntity, usage: UsageMetadata): number {
    ...
    // 入力トークン(プロンプトトークン - キャッシュトークン)
    tokenCount.cost += ((usage.promptTokenCount || 0) - (usage.cachedContentTokenCount || 0)) * aiPrice.inputPricePerUnit / unit;
    // 出力トークン
    tokenCount.cost += (usage.candidatesTokenCount || 0) * aiPrice.outputPricePerUnit / unit;
    // キャッシュトークン
    tokenCount.cost += (usage.cachedContentTokenCount || 0) * aiPrice.inputPricePerUnit / 10 / unit;

    if (tokenCount.model.startsWith('gemini-1.5') || tokenCount.model.startsWith('gemini-2.0')) {
        // gemini-1.5、2.0系は256kトークン超過分は2倍料金
        if (usage.totalTokenCount && usage.totalTokenCount > 128000) {
            const over256kTokens = usage.totalTokenCount - 128000;
            tokenCount.cost += over256kTokens * (aiPrice.inputPricePerUnit + aiPrice.outputPricePerUnit) / unit;
        } else { }
    } else { }
    if (tokenCount.model.startsWith('gemini-2.5') || tokenCount.model.startsWith('gemini-3.0')) {
        // gemini-1.5、2.0、2.5系は256kトークン超過分は2倍料金
        if (usage.totalTokenCount && usage.totalTokenCount > 200000) {
            const over256kTokens = usage.totalTokenCount - 200000;
            tokenCount.cost += over256kTokens * (aiPrice.inputPricePerUnit + aiPrice.outputPricePerUnit) / unit;
        } else { }
    } else { }
